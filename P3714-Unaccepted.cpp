...
intc N=2e5+10,C=1e4+10,inf=(1<<30);
int n,m,l,r;
struct edge {
    int to,c;
};
...
class SegmentTree {
    ...
    public:
    void build (int p=1,int l=0,int r=C) {
        tr[p]=-inf;
        if (l==r) return;
        int mid=(l+r)>>1;
        build(lc(p),l,mid);
    ...
    void update (int q,int w,int p=1,int l=0,int r=C) {
    ...
    int query (int ql,int qr,int p=1,int l=0,int r=C) {
    ...
    void clear (int clr,int p=1,int l=0,int r=C) {
        if (l==r) return tr[clr]=-inf,void();
        int mid=(l+r)>>1;
...
void solve (int u) {
    ...
    for (auto [v,c]:g[u]) {
        ...
        for (auto [dep,wei]:dis) res=max({res,stq.query(max(0,l-dep),max(0,r-dep))+wei,stp.query(max(0,l-dep),max(0,r-dep))+wei-cw[c]});
